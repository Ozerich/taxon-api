...
class ApiController extends CController
{
    ...
    public function actionStartSearch($order_id)
    {
        //$order = $this->getOrder();
        $order = Order::model()->findByPk($order_id);
    ...
    public function actionAddOrder()
    {
        ...
        if ($found) {
            $this->error('У вас уже есть активный заказ');
        }
    ...
    public function actionCheckOrder()
    {
        ...
        if(!in_array($order->status, array(Order::$STATUS_WAIT_CLIENT, Order::$STATUS_SEARCHING))){
            $this->error('Заказ просрочен', array('found' => 0));
        }

        if ($order->status == Order::$STATUS_SEARCHING) {
            $this->success('Заказ еще в поиске', array('found' => 0));
        }
